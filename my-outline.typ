...
#let my-outline(preface-state, preface-state-hide-parent, appendix-state, appendix-state-hide-parent, part-state, part-location, part-change, part-counter, sans-font, main-color, textSize1:none, textSize2:none, textSize3:none, textSize4:none) = {
  show outline.entry: it => {
    let preface-state = preface-state.at(it.element.location())
    let preface-state-hide-parent = preface-state-hide-parent.at(it.element.location())
    ...
    let numberingFormat = if preface-state != none {"I.1"} else {if appendix-state != none {"A.1"} else {"1.1"}}
    ...
    if it.level == 1 {
      ...
      if (part-change){
        v(0.7cm, weak: true)
        box(width: 1.1cm, fill: main-color.lighten(80%), inset: 5pt, align(center, text(size: textSize1, weight: "regular", fill: main-color.lighten(30%), numbering("I",part-counter.first()))))
        h(0.1cm)
        box(width: 100% - 1.2cm, fill: main-color.lighten(60%), inset: 5pt, align(center, link(part-location,text(size: textSize1, weight: "regular", part-state))))
        v(0.45cm, weak: true)
      }
      else{
        v(0.5cm, weak: true)
      }
      if (counterInt.first() == 1 and preface-state != none and not preface-state-hide-parent ){
        my-outline-row(insetSize: 2pt, textWeight: "regular", textSize: textSize2, textColor:main-color, number: none, title: preface-state, heading_page: heading_page, location: it.element.location())
        v(0.5cm, weak: true)
      }
      if (counterInt.first() == 1 and appendix-state != none and not appendix-state-hide-parent ){
        my-outline-row(insetSize: 2pt, textWeight: "regular", textSize: textSize2, textColor:main-color, number: none, title: appendix-state, heading_page: heading_page, location: it.element.location())
        v(0.5cm, weak: true)
      }
      if (appendix-state != none or preface-state != none) {
        my-outline-row(insetSize: 2pt, textWeight: "regular", textSize: textSize2, textColor:main-color, number: number, title: title, heading_page: heading_page, location: it.element.location())
      }
      else {
      my-outline-row(insetSize: 2pt, textWeight: "regular", textSize: textSize2, textColor:main-color, number: number, title: title, heading_page: heading_page, location: it.element.location())}
    }
...
#let my-outline-small(partTitle, appendix-state, part-state, part-location,part-change,part-counter, bold-font, main-color, textSize1:none, textSize2:none, textSize3:none, textSize4:none) = {
  show outline.entry: it => {
    let appendix-state = appendix-state.at(it.element.location())
    ...
    if (part-state == partTitle and counterInt.first() >0 and appendix-state==none){
      if it.level == 1 {
        set text(font: bold-font)
        v(0.5cm, weak: true)
        my-outline-row(insetSize: 1pt, textWeight: "regular", textSize: textSize2, textColor:main-color, number: number, title: title, heading_page: heading_page, location: it.element.location())
      }
      else if it.level ==2 {
