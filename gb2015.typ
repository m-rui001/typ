...
#let (init-gb7714, bibliography, footbib, bibitem, printbib, cite, set-cite-list) = gb7714(
  (
  ...
  //dash-in-pages: "～",
  //bib-label-style: "plain",
  //title-link: true,
  //bib-label-style: "quan",
  no-author: false,
  //no-others: true,
  full: true,
...
#sbi[
  ...
  #for i in range(1, 7) {
    set par(hanging-indent: 4.4em, first-line-indent: 0em)
    [#h(2em)\[#i\]#h(1em)#bibitem(label("6.1-1-" + str(i)))]
    v(0.85em, weak: true)
  }
]
// 试图让 [6] 的两行分到两页，但失败了
...
#sbi[
#phei[示例：]#bibitem("8.1.3", no-author: true)
]
...
#sbi[
  #for i in range(1, 6) {
    ...
    [#h(2em)#phei[示例#i：]#bibitem(label("8.5-" + str(i)))]
    v(0.85em, weak: true)
  }
]
#v(1.25em)
...
#sbi[
  #for i in range(1, 3) {
    ...
    [#h(2em)#phei[示例#i：]#bibitem(label("8.6-" + str(i)))]
    v(0.85em, weak: true)
  }
]
#v(1.25em)
...
#sbi[
  #for i in range(1, 4) {
    ...
    [#h(2em)#phei[示例#i：]#bibitem(label("8.8.1-" + str(i)))]
    v(0.85em, weak: true)
  }
]
#v(-.5em)
...
……改变社会规范也可能存在类似的“二阶囚徒困境”问题：尽管改变旧的规范对所有人都好，但个人理性选择使得没有人愿意率先违反旧的规范@10.1.3-1-1。……事实上，古希腊对轴心时代思想真正的贡献不是来自对民主的赞扬，而是来自对民主制度的批评，苏格拉底、柏拉图和亚里士多德3位贤圣都是民主制度的坚决反对者@10.1.3-1-2[260]。……柏拉图在西方世界的影响力是如此之大以至于有学者评论说，一切后世的思想都是一系列为柏拉图思想所作的脚注@10.1.3-1-3。……据《唐会要》记载，当时拆毁的寺院有4600余所，招提、兰若等佛教建筑4万余所，没收寺产，并强迫僧尼还俗达260500人。佛教受到极大的打击@10.1.3-1-2[326-329]。……陈登原先生的考证是非常精确的，他印证了《春秋说题辞》“黍者绪也，故其立字，禾人米为黍， 为酒以扶老，为酒以序尊卑，禾为柔物，亦宜养老”,指出：“以上谓等威之辨，尊卑之序，由于饮食荣辱。”@10.1.3-1-4
  ...
  #grid(
    columns: (2em, 1fr),
    row-gutter: 0.85em,
    [], [#printbib(label: "10.1.3-1-list", title: none, label-sep: 1em) ]
  )
...
// 切换新列表并指定著者-出版年制
#set-cite-list("10.2.1-list", style: "author-year")
#v(-.5em)
#sbi[
#phei[示例：]引用单篇文献
...
#grid(
    ...
    row-gutter: 0.85em,
    [], [// 打印该列表也用著者-出版年格式
#printbib(label: "10.2.1-list", style: "author-year", title: none)]
  )
]
#v(-.5em)
#heading(level: 4, outlined: false)[
...
#v(-.5em)
#sbi[
#set-cite-list("10.2.3-1-list", style: "author-year")
...
#grid(
    ...
    row-gutter: 0.85em,
    [], [// 打印该列表也用著者-出版年格式
#printbib(label: "10.2.3-1-list", style: "author-year", title: none)]
  )

#set-cite-list("10.2.3-2-list", style: "author-year")
...
#grid(
    ...
    row-gutter: 0.85em,
    [], [// 打印该列表也用著者-出版年格式
#printbib(label: "10.2.3-2-list", style: "author-year", title: none)]
  )
]
#v(-.5em)
#heading(level: 4, outlined: false)[
...
#v(-.5em)
#sbi[
#set-cite-list("10.2.4-list", style: "author-year")
...
#grid(
    columns: (2em, 1fr),
    row-gutter: 0.85em,
    [], [#printbib(label: "10.2.4-list", style: "author-year", title: none, sort-keys: ("10.2.4-1", "10.2.4-2", "10.2.4-3", "10.2.4-4"))]
  )
]

…………

#set-cite-list(none, style: "numeric")
