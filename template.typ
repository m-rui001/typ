...
#let template(
  ...
  // 设置正文和代码的字体
  set text(font: (fonts.en-font, fonts.zh-font), size: font_major_size, lang: "zh", region: "cn")
  show raw: set text(font: fonts.code-font, font_code_size)
  ...
  show link: it => {
    ...
    if it.body.has("text") and it.body.text in author-names {
      ...
      if link_underline and not link_accent-color {
        underline(stroke: (dash: "densely-dotted"), text(it))
      } else if not link_underline and link_accent-color {
        text(fill: blue, it)
      } else {
  ...
  // 文本高亮
  set highlight(fill: accent-color.lighten(text_high-light))
  ...
  set page(
    ...
    number-align: center,
    // 页边距
    margin: (x:document_page_margin_x, y:document_page_margin_y),
  ...
  set figure(
    numbering: (..nums) => locate(loc => {
      numbering("1.1.1.1", chaptercounter.at(loc).first(), ..nums)
    })
  )
  ...
  box(width: 100%, height: 40%)[
    // 显示论文的标题和描述。
    #align(right+bottom)[
      #text(font_title_size, weight: "bold")[#title]
      #parbreak()
      #if description != none {
        text(size: font_title-description_size, style: "italic")[#description]
      }
    ]
  ]
  ...
  box(width: 100%, height: 50%)[
    #align(right+top)[
        ...
        box(inset: (y: 10pt), {
          authors.map(author => {
            text(font_author_size, weight: "semibold")[
  ...
  box(width: 100%)[
    #if date != none {
      text(size: font_date_size, "最初写作于: ")
      text(
        size: font_date_size,
        fill: accent-color,
        weight: "semibold",
      ...
      parbreak()
      text(size: font_date_size, "最后更新于: ")
      text(
        size: font_date_size,
        fill: accent-color,
        weight: "semibold",
      ...
      text(size: font_date_size)[最后更新于：#h(5pt)] + text(
        size: font_date_size,
        fill: accen-color,
        weight: "semibold",
  ...
  if bibliography-file != none {
    pagebreak()
    show bibliography: set text(font_bibliography_size)
