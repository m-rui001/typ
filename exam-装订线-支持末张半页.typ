...
#set page(
  ...
  footer: context {
    ...
    let the-end = locate(<end>)
    // 最后一面是否两页都满了
    let last-full = the-end.position().x > 393mm / 2
    // 总页数修正量
    let total-delta = if last-full { 0 } else { -1 }
      ...
      columns: (1fr,) * 2,
      {
        counter(page).display(
          (n, total) => {
            ...
            footer-title
            h(2em)
            [第#(2 * n - 1)页，共#(2 * total + total-delta)页]
          },
          both: true,
        )
      },
      if the-end.page() == counter(page).get().at(0) and not last-full {
        // 最后一张的空页无需页码
      } else {
        counter(page).display(
          (n, total) => {
            footer-title
            h(1em)
            [第#(2 * n)页，共#(2 * total + total-delta)页]
          },
          both: true,
        )
      },
